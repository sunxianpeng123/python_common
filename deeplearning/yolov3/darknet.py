...
def parse_cfg(cfgfile):
    ...
    Returns a list of blocks. Each blocks describes a block in the neural network to be built. Block is represented as a dictionary in the list
    输入: 配置文件路径
    ...
    for line in lines:
        if line[0] == "[":                          # This marks the start of a new block
            if len(block) != 0:                     # If block is not empty, 说明是上一个块的信息还没有保存.
...
def create_modules(blocks):
    ...
    output_filters = []
    # 这里blocks是一个list，我们迭代 block[1:] 而不是blocks，因为blocks的第一个元素是一个net块，它不属于前向传播。
    for index, x in enumerate(blocks[1:]):
        ...
        if (x["type"] == "convolutional"):
            ...
            '''
            2. upsampling layer，上采样层，没有使用 Bilinear2dUpsampling，实际使用的为最近邻插值nearest
            '''
        ...
        # route层的作用：当layer取值为正时，输出这个正数对应的层的特征，如果layer取值为负数，输出route层向后退layer层对应层的特征
            ...
            if start > 0: 
                start = start - index
            # 若end>0，由于end= end - index，再执行index + end输出的还是第 end 层的特征
            if end > 0:
                end = end - index
            route = EmptyLayer()
