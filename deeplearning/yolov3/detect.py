...
import numpy as np
import cv2
from deeplearning.yolov3.util import *
import argparse
import os
import os.path as osp
from deeplearning.yolov3.darknet import Darknet
...
def arg_parse():
    ...
    parser.add_argument("--images", dest='images', help= "待检测图像目录",default="imgs", type=str)  # images是所有测试图片所在的文件夹
    parser.add_argument("--det", dest='det', help=  # det保存检测结果的目录
    "检测结果保存目录",default="det", type=str)
    ...
    parser.add_argument("--reso", dest='reso', help="网络输入分辨率. 分辨率越高,则准确率越高; 反之亦然.",
...
classes = load_classes(
    "data/coco.names")  # 将类别文件载入到我们的程序中，coco.names文件中保存的是所有类别的名字，load_classes()返回一个列表classes，每个元素是一个类别的名字
...
im_batches = list(map(prep_image, loaded_ims, [inp_dim for x in range(
    len(imlist))]))  # map函数将对应的元素作为参数传入prep_image函数，最终的所有结果也会组成一个列表(im_batches)，是BxCxHxW
im_dim_list = [(x.shape[1], x.shape[0]) for x in
               loaded_ims]  # 除了转换后的图像，我们也会维护一个列表im_dim_list用于保存原始图片的维度。一个元素对应一张图片的宽高,opencv读入的图片矩阵对应的是 HxWxC
...
for i, batch in enumerate(im_batches):
    ...
    if type(prediction) == int:
        ...
        for im_num, image in enumerate(imlist[i * batch_size: min((i + 1) * batch_size, len(imlist))]): # 计算图片在imlist中所对应的序号,即在所有图片中的序号
...
def write(x, results):
    ...
    cv2.putText(img, label, (c1[0], c1[1] + t_size[1] + 4), cv2.FONT_HERSHEY_PLAIN, 1, [225, 255, 255],
                1);  # 在图片上写文字，(c1[0], c1[1] + t_size[1] + 4)为字符串的左下角坐标
    return img
...
det_names = pd.Series(imlist).apply(
    lambda x: "{}/det_{}".format(args.det, x.split("/")[-1]))  # 每张图像都以「det_」加上图像名称的方式保存。我们创建了一个地址列表，这是我们保存我们的检测结果图像的位置。
...
print(
    "{:25s}: {:2.3f}".format("Average time_per_img", (end - load_batch) / len(imlist)))  # 从开始载入图片到所有结果处理完成，平均每张图片所消耗时间
print("----------------------------------------------------------")
