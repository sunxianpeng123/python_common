...
tb_increase_bigger_n as (
    select  t4.platform_id,
            ...
            -- t4.sales_number_add_sum_avg_percent,
            -- t4.sales_price_add_avg_percent,
            t4.statistics_date
...
tb_data as (
    ...
    WHERE
    	platform_id = {} 
    	AND (fansCount < 500 or fansCount > 50000 or totalViewerMax < 100) ) t3
    inner join(
    select 
    	t1.platform_id,
    	t1.room_id,
    ...
    WHERE
    	platform_id = {} 
    	AND cast ( statistics_date AS varchar ) IN {}) t1
    left join 
    (
    select 
    	platform_id,
...
WHERE
    ...
    print(sql_negative)
    # df = read_presto_data_test(sql_negative)
    # print(df.head(5))