...
def getData():
    ...
    x_train = np.stack([x_sample ** i for i in range(1, 4)], axis=1)  # n * 3,将 np.stack 列表罗列成矩阵样子
    y_train = torch.from_numpy(y_sample).double().unsqueeze(1)
    x_train = torch.from_numpy(x_train).float()
    print("x_train shape = {},, y_train shape = {}，，y_sample shape = {}".format(x_train.shape, y_train.shape,
                                                                                y_sample.shape))
...
if __name__ == '__main__':
    ...
    for epoch in range(epochs):
        ...
        b.grad.data.zero_()
        if epoch % 10 == 0:
            print("w.shape = {}\nb = {}\nb.shape = {}".format(w.shape, b, b.shape))
            # print("w = {}\nw.shape = {}\nb = {}\nb.shape = {}".format(w, w.shape, b, b.shape))
