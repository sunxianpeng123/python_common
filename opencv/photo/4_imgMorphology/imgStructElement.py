...
# ==============================================================================
def getSomeStructShape():
    """
    几种不同的结构化矩阵
    :return:
    """
    # Rectangular Kernel 矩形核
    shape1 = cv2.getStructuringElement(cv2.MORPH_RECT, (5, 5))
    # Elliptical Kernel 椭圆形核
    shape2 = cv2.getStructuringElement(cv2.MORPH_RECT, (5, 5))
    # Cross-shaped Kernel 十字星核
    shape3 = cv2.getStructuringElement(cv2.MORPH_CROSS, (5, 5))

def imgStructElement(gray):
    """
    结构化元素
    :param img:
    :return:它是输入图像和图像开运算之间的区别。礼帽（image） = image - 开运算(image)，得到前景图（白色区域）外面的毛刺噪声（得到噪声）。
    """
