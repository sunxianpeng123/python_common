...
def tf_sort_argsort():
    ...
    # 默认axis为最后一个维度，此处即axis=1，表示按照第二个维度，每组数据有三个，在每组数据中排序
    d = tf.sort(c)
...
def tf_topk_accuracy():
    ...
    # 真实值，所有样本的真实标签
    y = tf.constant([2, 0])
    # 将shape为（2，）的tensor转为 shape=（3,2）列的tensor，方便与top_k_indices进行比较
    ...
    # 指定 top_k_indices 的维度顺序，即perm指的是top_k_indices中维度的下标
    # 在进行转置之前，top_k_indices的每行表示的是对同一个样本预测的标签，
    # 在进行转置之后，top_k_indices的每行表示的是对所有样本的预测标签，刚好和y相对应
    top_k_indices = tf.transpose(top_k_indices,perm=[1,0])
    # 比较预测值和真实值，得到boolean矩阵
    correct = tf.equal(top_k_indices,y_)

    res = []
    for i in range(1,k + 1):
        # 取出前index行，并将前k行tensor进行转置，-1表示系统自动求解转置的形状大小
